...
def qa(llm_choice: str, question: str, top_k: int, temperature: float, max_tokens: int):
    ...
    if not context.strip():
        return "No relevant chunks retrieved. Try increasing top-k.", gr.update(choices=source_choices, value=None)

    ans = call_llm(llm_choice, question, context, float(temperature), int(max_tokens))
    return ans, gr.update(choices=source_choices, value=None)
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column():
            ...
            gr.Markdown(
                "How does this app work - Ask a question → adjust settings → click Submit → review answer → click a source to open its file location."
                + " The intended files need to be retrieved, are required to be ingested by using relevant ingestion code files. Then the questions can be asked using this GUI."
            )
    ...
    submit_btn.click(
        ...
        outputs=[answer, source_selector],
